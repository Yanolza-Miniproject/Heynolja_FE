...
describe('MyOrderList 컴포넌트', () => {
  ...
  test('결제 내역이 없는 경우 "아직 결제 완료한 숙소가 없어요!" 메시지가 나타나야 함', () => {
    // useGetMyOrder 훅을 모킹하여 빈 데이터를 반환하도록 설정
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    jest.spyOn(require('../../../../hooks/usePayment'), 'useGetMyOrder').mockReturnValueOnce({
      data: { data: { data: [] } },
    });

    render(<MyOrderList />, { wrapper: createWrapper() });
    
    // "아직 결제 완료한 숙소가 없어요!" 메시지가 있는지 확인
    const noOrderMessage = screen.queryByText('아직 결제 완료한 숙소가 없어요!');
